'''
CoordConverter library, for converting grid coordinates in these forms:
mgrs, latlong (dec & deg/min/sec), utm
Author: Tyler Weaver

v0.1 improvements for output lat,long forms (dms and dd)
--------
...
class CoordTranslator:
    def AsMGRS(self, coord, precision = 5, internal = False, spaces = True):
    ...
    def AsLatLong(self, coord, dms = False, aslist = True, internal = False):
        ...
        if dms:
            ...
            if aslist:
                return [ lond, lonmin, lonsec, latd, latmin, latsec ]
    ...
    def ParseCoord(self, coord):
        ...
        if type(coord) == type([]):
            ...
            if self.re_mgrs.match(coord):
                ...
                out.append(float(coord[span[0]:span[1]]))
                return out, 'UTM'
            elif self.re_decdeg.match(coord):
                # Longitude
                span = re.compile('[+-]?[\d.]+,').search(coord).span()
                lon = float(coord[span[0]:span[1]-1])
                # Latitude
                span = re.compile('[+-]?[\d.]+').search(coord, span[1]+1).span()
                lat = float(coord[span[0]:span[1]])
                return [lon, lat], 'LL'
	...
	[ 23, 'WGS-84', 6378137, 0.00669438]]
        ...
        self.re_decdeg = re.compile('[+-]?[0-9.]+,[\s]?[+-]?[0-9.]+') #decimal degree format
    ...
    def LLtoUTM(self, latlong):
        ...
        # Returns as list
        return [ZoneNumber, self._UTMLetterDesignator(Lat), (UTMEasting), (UTMNorthing)]
...
#
#     print a.AsMGRS([34.213352, 70.291130])
#     print a.AsLatLong([34.213352, 70.291130])
#     print a.AsUTM([34.213352, 70.291130])
#
#